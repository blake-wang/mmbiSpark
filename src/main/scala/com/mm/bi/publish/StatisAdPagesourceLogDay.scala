...
/**
  * statis_ad_pagesource_log_day
  * Created by kequan on 3/28/18.
  */
object StatisAdPagesourceLogDay {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sql_hive =
      """
        ...
        |LEFT JOIN (
        |	SELECT b.Source,b.GameId ,trim(a.domainName) domainName
        ...
        |	GROUP BY b.Source, b.GameId,trim(a.domainName) HAVING pvcount >10
        |) AS b ON a.Source=b.Source AND a.GameId=b.GameId
        |WHERE domainName IS NOT NULL AND domainName <>''
      """.stripMargin
    ...
    val sql_mysql =
      """
        |insert into statis_ad_pagesource_log_day(DayTime,GameId,Source,domainName,IpCount,PvCount,UvCount,windowsCount,IOSCount,AndroidCount) values(?,?,?,?,?,?,?,?,?,?)
